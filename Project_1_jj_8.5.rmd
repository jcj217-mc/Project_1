...
```{r}
diversity_new$name[diversity_new$name == "NewSchool of Architecture and Design"] <- "NewSchool_Arch_Design"
```

```{r}
diversity_new$name[diversity_new$name == "Western International University"] <- "Western_Intl_Univ"
```

```{r}
diversity_new$name[diversity_new$name == "Schiller International University"] <- "Schiller_Intl_Univ"
```

```{r}
diversity_new$name[diversity_new$name == "National Paralegal College"] <- "Natl_Paralegal_College"
```


```{r}
diversity_new$name[diversity_new$name == "WestCoast University"] <- "West_Coast_Univ"
```
...
```{r}
diversity_new$name[grepl('West Coast University', diversity_new$name)] <- 'West_Coast_Univ'
```
...
diversity_new$name[23438]
diversity_new$name[24683]
diversity_new$name[27995]
diversity_new$name[30703]
```
```{r}
diversity_new$name[grepl('Brookline College', diversity_new$name)] <- 'Brookline_College'
```

```{r}
diversity_new$name[37182]
diversity_new$name[38958]
```
```{r}
diversity_new$name[grepl('University of Houston', diversity_new$name)] <- 'Univ_of_Houston'
```

```{r}
diversity_new$name[grepl('University of Colorado', diversity_new$name)] <- 'Univ_of_Colorado'
```


```{r}
diversity_new$name[grepl('University of Massachusetts', diversity_new$name)] <- 'Univ_of_Massachusetts'
```


```{r}
diversity_new$name[grepl('Arizona College', diversity_new$name)] <- 'Arizona_College'
```

```{r}
diversity_new$name[grepl('University of Idaho', diversity_new$name)] <- 'Univ_of_Idaho'
```

```{r}
diversity_new$name[grepl('Southwestern Community College (Iowa)', diversity_new$name)] <- 'Southwestern_Comm_College(Iowa)'
```

```{r}
diversity_new$name[grepl('Clover Park Technical College', diversity_new$name)] <- 'Clover_Park_Tech_College'
```

```{r}
diversity_new$name[grepl('Clark College', diversity_new$name)] <- 'Clark_College'
```

```{r}
diversity_new$name[grepl('City University of New York Hunter College', diversity_new$name)] <- 'City_Univ_NewYork_Hunter_College'
```


```{r}
diversity_new$name[grepl('San Diego City College', diversity_new$name)] <- 'San_Diego_City_College'
```
